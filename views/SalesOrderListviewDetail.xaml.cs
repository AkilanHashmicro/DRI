...
namespace SalesApp.views
{
    public partial class SalesOrderListviewDetail : PopupPage
    {
        ...
        public SalesOrderListviewDetail( SalesOrder item )
        {
            ...
            orderListview.HeightRequest = item.order_line.Count * 70;
        ...
        {
           // await Navigation.PushPopupAsync(new AttachmentPopupPage(attachres, sale_order_id));

            var currentpage = new LoadingAlert();
            await PopupNavigation.PushAsync(currentpage);
            attachres = Controller.InstanceCreation().getfileAtachment(sale_order_id, "crm.lead");
