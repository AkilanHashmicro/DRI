...
namespace SalesApp.OdooRpc
{
    class OdooRPC
    {
        ...
        public string odooUpdate_saleorder(string model, string method, int sale_id, Dictionary<string, dynamic> vals)
        {
            if (this.Uid == 0)
            {
                this.Uid = App.userid_db;
        ...
        public string odooUpdateGpsData(string model, string method, float lat, float longitude, string id, string time)
        {
            JsonRpcRequestParameter parameters = new JsonRpcRequestParameter("object", "execute", new object[] { this.Database, this.Uid, Settings.UserPassword, model, method, lat, longitude, id, time });
            string result = odooServerCall<string>(jsonRpcUrl, parameters);
            return result;
        ...
        public string clearGpsData(string model, string method, string id)
        {
